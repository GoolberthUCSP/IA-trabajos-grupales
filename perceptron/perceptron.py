...
def train(training_set):
    ...
    while True:
        error_count = 0
        print(f'Iteración: {iteration}, Pesos: {weights}, Recuento de errores: {error_count}')
        for input_vector, desired_output in training_set:
            result = dot_product(input_vector, weights) > threshold
            error = desired_output - result
            print("Resultado: {0} Error: {1}".format(result, error))
            if error != 0:
                error_count += 1
...
def main():
    create_perceptrons()
    while True:
        sensor_data = input("Ingrese los datos del sensor: ")
        sensor_data = [int(x) for x in sensor_data.split()]
